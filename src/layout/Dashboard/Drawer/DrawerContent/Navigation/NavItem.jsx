...
export default function NavItem({ item, level }) {
  ...
  let iconSelectedColor = 'primary.main';
  let hasPendingBooking = false;
  if (item.title === 'Bookings') {
    hasPendingBooking = bookings.some((booking) => booking.bookingStatus === 'Pending');

    if (hasPendingBooking) {
      textColor = 'success.main';
      iconSelectedColor = 'success.main';
    }
  }
        ...
        <ListItemText
          primary={
            <Typography variant="h6" sx={{ color: isSelected ? iconSelectedColor : textColor }}>
              {item.title}
              {hasPendingBooking && <Chip label="New" color="success" size="small" sx={{ marginLeft: 1, borderRadius: '16px' }} />}
            </Typography>
          }
        />
      )}
